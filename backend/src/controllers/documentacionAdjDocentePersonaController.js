...
const getAllDocumentacionAdjDocentePersona = async (req, res) => {
  try {
    ...
    const documentacion = await prisma.documentacionAdjDocentePersona.findMany({
      where: whereClause,
      include: {
        persona: true
      },
    ...
    console.error('Error al obtener documentación de docentes:', error);
...
const getDocumentoById = async (req, res) => {
  try {
    const { id } = req.params;
    const documento = await prisma.documentacionAdjDocentePersona.findUnique({
...
const createDocumento = async (req, res) => {
  try {
    ...
    const prefix = `docentes/${id_persona}/`;
    ...
    const documento = await prisma.documentacionAdjDocentePersona.create({
      data: {
...
const updateDocumento = async (req, res) => {
  try {
    ...
    const documento = await prisma.documentacionAdjDocentePersona.update({
      where: { id_documento: parseInt(id) },
      data: {
        tipo_documento,
        nombre_archivo,
...
const deleteDocumento = async (req, res) => {
  try {
    ...
    await prisma.documentacionAdjDocentePersona.delete({
      where: { id_documento: parseInt(id) }
    });
...
module.exports = {
  getAllDocumentacionAdjDocentePersona,
  getDocumentoById,
  createDocumento,
  updateDocumento,
  deleteDocumento,
